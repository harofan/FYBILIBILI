...
#import "FYViewControllerHeader.h"
#import "FYFlowlayout.h"

@interface FYViewController ()<UICollectionViewDataSource,UICollectionViewDelegate>
...
{
    // 初始化标题高度
    _titleHeight = FYTitleScrollViewH;
...
- (UIView *)bottomLine
{
    if (!_bottomLine) {
        ...
        bottomLine.backgroundColor = FYMainColor;
...
- (UIScrollView *)contentScrollView
{
    if (_contentScrollView == nil) {
        ...
        // 创建布局
        FYFlowlayout *layout = [[FYFlowlayout alloc] init];
...
- (void)setIsfullScreen:(BOOL)isfullScreen
{
    ...
    self.contentView.frame = CGRectMake(0, 0, FYScreenW, FYScreenH);
...
- (void)viewDidLayoutSubviews
{
    ...
    CGFloat contentY = self.navigationController?FYNavBarH : [UIApplication sharedApplication].statusBarFrame.size.height;
    CGFloat contentW = FYScreenW;
    CGFloat contentH = FYScreenH - contentY;
    // 设置整个内容的尺寸
    ...
    // 设置标题滚动视图frame
    // 计算尺寸
    CGFloat titleH = _titleHeight?_titleHeight:FYTitleScrollViewH;
    ...
    self.contentScrollView.frame = _isfullScreen?CGRectMake(0, 0, contentW, FYScreenH) :CGRectMake(0, contentScrollY, contentW, self.contentView.height - contentScrollY);
...
- (void)setUpTitleWidth
{
    ...
    CGFloat titleMargin = (FYScreenW - totalWidth) / (count + 1);
...
- (void)setUpAllTitle
{
    ...
    for (int i = 0; i < count; i++) {
        ...
        UILabel *label = [[FYTitleLable alloc] init];
    ...
    if (_isBisectedWidthUnderLineAndTitle) {
        _titleScrollView.contentSize = CGSizeMake(FYScreenW, 0);
    } else {
        _titleScrollView.contentSize = CGSizeMake(CGRectGetMaxX(lastLabel.frame), 0);
    }
    
    _titleScrollView.showsHorizontalScrollIndicator = NO;
    _contentScrollView.contentSize = CGSizeMake(count * FYScreenW, 0);
    
}

#pragma mark - 标题效果渐变方法
// 设置标题颜色渐变
- (void)setUpTitleColorGradientWithOffset:(CGFloat)offsetX rightLabel:(FYTitleLable *)rightLabel leftLabel:(FYTitleLable *)leftLabel
{
    if (_isShowTitleGradient == NO) return;
    
    // 获取右边缩放
    CGFloat rightSacle = offsetX / FYScreenW - leftLabel.tag;
...
- (void)setUpTitleScaleWithOffset:(CGFloat)offsetX rightLabel:(UILabel *)rightLabel leftLabel:(UILabel *)leftLabel
{
    ...
    CGFloat scaleTransform = _titleScale?_titleScale:FYTitleTransformScale;
...
- (void)setUpUnderLineOffset:(CGFloat)offsetX rightLabel:(UILabel *)rightLabel leftLabel:(UILabel *)leftLabel
{
    ...
    // 计算当前下划线偏移量
    CGFloat underLineTransformX = offsetDelta * centerDelta / FYScreenW;
    
    // 宽度递增偏移量
    CGFloat underLineWidth = offsetDelta * widthDelta / FYScreenW;
    
    if (_isBisectedWidthUnderLineAndTitle) {
        
        NSUInteger count = self.childViewControllers.count;
        
        self.underLine.width = FYScreenW / count;
...
- (void)setUpCoverOffset:(CGFloat)offsetX rightLabel:(UILabel *)rightLabel leftLabel:(UILabel *)leftLabel
{
    ...
    // 计算当前下划线偏移量
    CGFloat coverTransformX = offsetDelta * centerDelta / FYScreenW;
    
    // 宽度递增偏移量
    CGFloat coverWidth = offsetDelta * widthDelta / FYScreenW;
...
- (void)titleClick:(UITapGestureRecognizer *)tap
{
    ...
    if (vc.view) {
        // 发出通知点击标题通知
        [[NSNotificationCenter defaultCenter] postNotificationName:FYDisplayViewClickOrScrollDidFinshNote  object:vc];
        
        // 发出重复点击标题通知
        if (_selIndex == i) {
            [[NSNotificationCenter defaultCenter] postNotificationName:FYDisplayViewRepeatClickTitleNote object:vc];
        }
    }
...
- (void)selectLabel:(UILabel *)label
{
    ...
    for (FYTitleLable *labelView in self.titleLabels) {
...
- (void)setUpUnderLine:(UILabel *)label
{
    ...
    CGFloat underLineH = _underLineH?_underLineH:FYUnderLineH;
...
- (void)setLabelTitleCenter:(UILabel *)label
{
    ...
    // 设置标题滚动区域的偏移量
    CGFloat offsetX = label.center.x - FYScreenW * 0.5;
    ...
    // 计算下最大的标题视图滚动区域
    CGFloat maxOffsetX = self.titleScrollView.contentSize.width - FYScreenW + _titleMargin;
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    ...
    NSInteger screenWInt = FYScreenW;
    
    NSInteger extre = offsetXInt % screenWInt;
    if (extre > FYScreenW * 0.5) {
        // 往右边移动
        offsetX = offsetX + (FYScreenW - extre);
        _isAniming = YES;
        [self.contentScrollView setContentOffset:CGPointMake(offsetX, 0) animated:YES];
    }else if (extre < FYScreenW * 0.5 && extre > 0){
        _isAniming = YES;
        // 往左边移动
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    // 获取左边角标
    NSInteger leftIndex = offsetX / FYScreenW;
    
    // 左边按钮
    FYTitleLable *leftLabel = self.titleLabels[leftIndex];
    
    // 右边角标
    NSInteger rightIndex = leftIndex + 1;
    
    // 右边按钮
    FYTitleLable *rightLabel = nil;
